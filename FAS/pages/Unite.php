...
{   
 $sql = "SELECT  rr.id,Role_Name from roleresources rr inner join Role ro ON rr.Role_Id=ro.id INNER join  resources re ON rr.Resoources_Id=re.Id      inner JOIN authorities a ON a.RR_Id=rr.Id where re.Resources_Name='Unite' and a.User_ID=".$_SESSION['user_Id'];
 $result = mysqli_query($db, $sql);
 $count = mysqli_num_rows($result);  
    if ($count > 0) {
...
else
{
 ...
 $count = mysqli_num_rows($result);  
    ...
    // output data of each row
    while($row = mysqli_fetch_assoc($result)) {
    if($row["status"]==1){$g='enabled ';}else{$g='disabled';} 
        echo "<tr id='".$row["Id"]."' ><td>" . "<input type='radio'  class='btn btn-warning btn-lg btn-block' name='selectedrow' " . "value='".$row["Id"]."'/>". "</td><td data-value='Name' >". $row["Name"]. "</td><td data-value='Description' >".  $row["Description"]. "</td><td data-value='status'>". $g."</td></tr>";
    }
  }
else
{
...
if(isset($_POST['txtn'])&&isset($_POST['txtd'])&&isset($_POST['status'])){
     $sql = "INSERT INTO unite(Name,Description,status)
    VALUES('".$_POST["txtn"]."','".$_POST["txtd"]."','".$_POST["status"]."')";
if (mysqli_query($db, $sql)) {
     $_SESSION["msg"] ="new unite is added";
     $_SESSION["typ"] ='s'; 
} else {
     $_SESSION["msg"] ="err:unite not added";
     $_SESSION["typ"] ='w';
}

    }     /* end add unite */ /*delete unite*/
      }elseif($_POST['submit']=='delete'){
       $id= $_POST["hf"];
       $sql = "DELETE FROM unite WHERE id=$id";
...
if (mysqli_query($db, $sql)) {
     ...
     $_SESSION["typ"] ='w';
      ...
      } /* end delete unite */ /*edit unite*/
      elseif($_POST['submit']=='update'){
           $id= $_POST["hf"];
         if(isset($_POST['txtn'])&&isset($_POST['txtd'])&&isset($_POST['status'])){
     $sql = "UPDATE unite SET  Name='".$_POST["txtn"]."',Description='".$_POST["txtd"]."',status='".$_POST["status"]."' where Id='$id'";

if (mysqli_query($db, $sql)) {
    
      $_SESSION["msg"] ="unite information is updated";
      $_SESSION["typ"] ='s';
} else {
      $_SESSION["msg"] ="err:unite not update";
      $_SESSION["typ"] ='w';
            }
         } 
      }/*end edit unite*/
...
<!--add row -->
   <?php
                               if($dbins){ echo "<input type='button' id='btnmodel' class='btn btn-primary' value='new Unite' data-toggle='modal' data-target='#myModal'/>";}    
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <!-- body -->                                                                                          
                                                 ...
                                                 <label >Name:</label><input class="form-control "  type="text" id="txtn" name="txtn"  placeholder="unite name" />
                                                 <label >Description:</label><input class="form-control"  type="text" id="txtd" name="txtd" placeholder="unite Description" />
                                                 <label >status:</label>
                                                 <select class="form-control"  name="status">
                                                 <option value="0" id="om">enabled</option>
                                                 <option value="1" id="of">disabled</option>
                                                 </select>    
                                                  </div>
