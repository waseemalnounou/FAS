...
{   
 $sql = "SELECT  rr.id,Role_Name from roleresources rr inner join Role ro ON rr.Role_Id=ro.id INNER join  resources re ON rr.Resoources_Id=re.Id      inner JOIN authorities a ON a.RR_Id=rr.Id where re.Resources_Name='gpio' and a.User_ID=".$_SESSION['user_Id'];
 $result = mysqli_query($db, $sql);
 $count = mysqli_num_rows($result);  
    if ($count > 0) {
...
{
 $sql = "SELECT Id,Name FROM raspberry";
 $result = mysqli_query($db, $sql);
 $count = mysqli_num_rows($result);  
    if ($count > 0) {
    // output data of each row
...
{
 $sql = "SELECT *,(select Name from raspberry r where g.raspberry_id=r.id) as Name  FROM GPIO g";
 $result = mysqli_query($db, $sql);
 $count = mysqli_num_rows($result);  
    if ($count > 0) {
    // output data of each row
    while($row = mysqli_fetch_assoc($result)) {
        if($row["status"]==1){$g='out';}else{$g='in';} 
        echo "<tr id='".$row["Id"]."' ><td>" . "<input type='radio'  class='btn btn-warning btn-lg btn-block' name='selectedrow' " . "value='".$row["Id"]."'/>". "</td><td data-value='Name' >". $row["Name"]. "</td><td data-value='GN' >".  $row["GPIO_Number"]. "</td><td data-value='Des'>" .  $row["Description"]. "</td><td data-value='status'>". $g."</td></tr>";
    }
  }
else
{
...
if(isset($_POST['NGPIO'])&&isset($_POST['txtd'])&&isset($_POST['status'])&isset($_POST['Name'])){
     $sql = "INSERT INTO GPIO(raspberry_id,GPIO_Number,status,Description,Type)
    VALUES((select Id from raspberry r where r.Name='".$_POST["Name"]."'),'".$_POST["NGPIO"]."','".$_POST["status"]."','".$_POST["txtd"]."',0)";
if (mysqli_query($db, $sql)) {
     $_SESSION["msg"] ="new GPIO is added";
     $_SESSION["typ"] ='s'; 
} else {
     $_SESSION["msg"] ="err:GPIO not added";
     $_SESSION["typ"] ='w';
}

    }     /* end add GPIO */ /*delete GPIO*/
      }elseif($_POST['submit']=='delete'){
       $id= $_POST["hf"];
       $sql = "DELETE FROM GPIO WHERE id=$id";
...
if (mysqli_query($db, $sql)) {
     ...
     $_SESSION["typ"] ='w';
      ...
      } /* end delete GPIO */ /*edit GPIO*/
      elseif($_POST['submit']=='update'){
           $id= $_POST["hf"];
         if(isset($_POST['NGPIO'])&&isset($_POST['Name'])&&isset($_POST['txtd'])&isset($_POST['status'])){
     $sql = "UPDATE GPIO SET  raspberry_id=(select Id from raspberry r where r.Name='".$_POST["Name"]."'),GPIO_Number='".$_POST["NGPIO"]."',Description='".$_POST["txtd"]."',status='".$_POST["status"]."' where Id='$id'";

if (mysqli_query($db, $sql)) {
    
      $_SESSION["msg"] ="GPIO information is updated";
      $_SESSION["typ"] ='s';
} else {
      $_SESSION["msg"] ="err:GPIO not update";
      $_SESSION["typ"] ='w';
            }
         } 
      }/*end edit GPIO*/
            ...
            <ul class="nav navbar-top-links navbar-right">
                ...
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu dropdown-user">
                        <li><a href="sys.php"><i class="fa fa-user fa-fw"></i> User Profile</a>
                        </li>
                        <li><a href="userprofile.php"><i class="fa fa-gear fa-fw"></i> Settings</a>
                        </li>
                        <li class="divider"></li>
...
<!--add row -->
   <?php
                               if($dbins){ echo "<input type='button' id='btnmodel' class='btn btn-primary' value='new GPIO' data-toggle='modal' data-target='#myModal'/>";}    
...
<div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <!-- body -->                                                                                          
                                                 ...
                                                 <label >GPIO Number:</label><input class="form-control " min="1" max="27"  type="number" id="NGPIO" name="NGPIO"  placeholder="GPIO Number" />
                                                 <label >GPIO Description:</label><input class="form-control "  type="text" id="txtd" name="txtd"  placeholder="GPIO Description" />
                                                 <label >GPIO status:</label>
                                                 <select class="form-control "  name="status">
                                                 <option value="1" id="om">Out</option>
                                                 <option value="0" id="of">In</option>
                                                 </select>
                                                  </div>
   ...
   $("#edt").click(function () {
                 ...
                 case 'Name':
                 $("#Name").val($(this).text()); break;
                 case 'GN':
                 $("#NGPIO").val($(this).text()); break;
                 case 'Des':
