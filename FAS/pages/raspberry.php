...
{   
 $sql = "SELECT  rr.id,Role_Name from roleresources rr inner join Role ro ON rr.Role_Id=ro.id INNER join  resources re ON rr.Resoources_Id=re.Id      inner JOIN authorities a ON a.RR_Id=rr.Id where re.Resources_Name='raspberry' and a.User_ID=".$_SESSION['user_Id'];
 $result = mysqli_query($db, $sql);
 $count = mysqli_num_rows($result);  
    if ($count > 0) {
...
else
{
 ...
 $count = mysqli_num_rows($result);  
    ...
    // output data of each row
    while($row = mysqli_fetch_assoc($result)) {
        echo "<tr id='".$row["Id"]."' ><td>" . "<input type='radio'  class='btn btn-warning btn-lg btn-block' name='selectedrow' " . "value='".$row["Id"]."'/>". "</td><td data-value='Name' >". $row["Name"]. "</td><td data-value='Description' >".  $row["Description"]. "</td></tr>";
    }
  }
else
{
...
if(isset($_POST['txtn'])&&isset($_POST['txtd'])){
     $sql = "INSERT INTO raspberry(Name,Description)
    VALUES('".$_POST["txtn"]."','".$_POST["txtd"]."')";
if (mysqli_query($db, $sql)) {
     $_SESSION["msg"] ="new raspberry is added";
     $_SESSION["typ"] ='s'; 
} else {
     $_SESSION["msg"] ="err:raspberry not added";
     $_SESSION["typ"] ='w';
}

    }     /* end add raspberry */ /*delete raspberry*/
      }elseif($_POST['submit']=='delete'){
       $id= $_POST["hf"];
       $sql = "DELETE FROM raspberry WHERE id=$id";
...
if (mysqli_query($db, $sql)) {
     ...
     $_SESSION["typ"] ='w';
      ...
      } /* end delete raspberry */ /*edit raspberry*/
      elseif($_POST['submit']=='update'){
           $id= $_POST["hf"];
         if(isset($_POST['txtn'])&&isset($_POST['txtd'])){
     $sql = "UPDATE raspberry SET  Name='".$_POST["txtn"]."',Description='".$_POST["txtd"]."' where Id='$id'";

if (mysqli_query($db, $sql)) {
    
      $_SESSION["msg"] ="raspberry information is updated";
      $_SESSION["typ"] ='s';
} else {
      $_SESSION["msg"] ="err:raspberry not update";
      $_SESSION["typ"] ='w';
            }
         } 
      }/*end edit raspberry*/
    ...
    <script src="/FAS/js/index.js"></script>
        ...
        <div id="page-wrapper">
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    <h1 class="page-header">raspberrys</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                        ...
                        <div class="panel-body">
                            <table style="width: 100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                <thead>
                                    <tr>
                                         <th>raspberry</th>
                                        <th>raspberry Name</th>
                                         <th>raspberry Description</th>
...
<!--add row -->
   <?php
                               if($dbins){ echo "<input type='button' id='btnmodel' class='btn btn-primary' value='new raspberry' data-toggle='modal' data-target='#myModal'/>";}    
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <!-- body -->                                                                                          
                                                 ...
                                                 <label >Name:</label><input class="form-control "  type="text" id="txtn" name="txtn"  placeholder="raspberry name" />
                                                 <label >Description:</label><input class="form-control"  type="text" id="txtd" name="txtd" placeholder="raspberry Description" />
