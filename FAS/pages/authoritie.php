...
{   
 $sql = "SELECT  rr.id,Role_Name from roleresources rr inner join Role ro ON rr.Role_Id=ro.id INNER join  resources re ON rr.Resoources_Id=re.Id      inner JOIN authorities a ON a.RR_Id=rr.Id where re.Resources_Name='authoritie' and a.User_ID=".$_SESSION['user_Id'];
 $result = mysqli_query($db, $sql);
 $count = mysqli_num_rows($result);  
    if ($count > 0) {
...
else
{
 ...
 $count = mysqli_num_rows($result);  
    ...
    // output data of each row
    while($row = mysqli_fetch_assoc($result)) {
          echo "<option id='".$row["Id"]."'  value='".$row["Id"]."'>".$row["First_Name"]." ".$row["Last_Name"]."</option>";
    }
  }
else
{
 ...
 function  initrole($db,$selecteduser){ 
...
else
{
 ...
 $sql = "SELECT rr.Id,re.Resources_Name,ro.Role_Name FROM `roleresources` rr inner JOIN resources re ON  rr.Resoources_Id=re.Id INNER JOIN role ro ON ro.Id=rr.Role_Id where rr.Id NOT IN(SELECT RR_Id FROM authorities where User_ID=".$selecteduser.")";
 $result = mysqli_query($db, $sql);
 $count = mysqli_num_rows($result);  
    if ($count > 0) {
    // output data of each row
    while($row = mysqli_fetch_assoc($result)) {
        echo "<option value='".$row["Id"]."'  >".$row["Resources_Name"]." Role :".$row["Role_Name"]."</option>";
    }
  }
else
{
 ...
 function  inituserrole($db,$selecteduser){   
     if($selecteduser!=NULL){                
    if (!$db) {
    die("Connection failed: " . mysqli_connect_error());
}
...
{
     echo $selecteduser;
 $sql = "SELECT  rr.Id,re.Resources_Name,ro.Role_Name from roleresources rr inner join Role ro ON rr.Role_Id=ro.id INNER join  resources re ON rr.Resoources_Id=re.Id      inner JOIN authorities a ON a.RR_Id=rr.Id where a.User_ID=".$selecteduser;
 $result = mysqli_query($db, $sql);
 $count = mysqli_num_rows($result);  
    if ($count > 0) {
    // output data of each row
...
?>                
                        ...
                        <li>
                            ...
                            <ul class="nav nav-second-level">
                              <li>
                                    <a href="#">authoritie <span class="fa arrow"></span></a>
    ...
    <script src="/FAS/js/index.js"></script>
        ...
        <div id="page-wrapper">
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    <h1 class="page-header">authorities</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            authorities Information |  plese select a user
                             <form  method="post">         
                                 <?php echo "<input type='hidden' value='".$selecteduser."' id='hfuser'/>" ?>
                            <select onchange="javascript: form.submit();" name="userlist" class="form-control">
                                <option value="null">plese select</option>
                            <?php
 inituserlist($db);
?>
                            </select>
  ...
  <select name="from1[]" id="rlist" class="multi js-multiselect form-control" name="" multiple="multiple" size='20'>
<?php initrole($db,$selecteduser); ?>
    </select>   
                            </div>
                             <div class="col-xs-2">
                                 </br></br>
        <button type="button" id="RSA" style="background-color: #fff" class="btn btn-block"><i class="glyphicon glyphicon-forward"></i></button>
        <button type="button" id="RS1" style="background-color: #fff" class="btn btn-block"><i class="glyphicon glyphicon-chevron-right"></i></button>
        <button type="button" id="LS1" style="background-color: #fff" class="btn btn-block"><i class="glyphicon glyphicon-chevron-left"></i></button>
        <button type="button" id="LSA" style="background-color: #fff" class="btn btn-block"><i class="glyphicon glyphicon-backward"></i></button>
    </div>
                            <div class="col-xs-5">
                                    </br>
  <select name="from2[]" id="urlist" class="multi js-multiselect form-control"  name="" multiple="multiple" size='20'>
<?php
    inituserrole($db,$selecteduser);
?>
...
<!--add row -->
                                ...
                                <?php
                        //     if($dbins){ echo "<input type='button' id='btnins' class='btn btn-success' value='conferm' />";}    
                           //    if($dbdlt){ echo"  <input type='button'  id='btre' class='btn btn-danger'  value='<<'/>";}
                                ?>
.<hr/>
    ...
    <script>
        $(document).ready(function () {
            $("#RS1").click(function () {
                var rr = [];
                $('#rlist :selected').each(function (i, selected) {
                    var data = {
                        id: $(selected).val(),
                        text: $(selected).text()
                    };
                    ...
                    $('#urlist').append(newOption).trigger('change');
                    $(selected).remove();
                    rr[i] = $(selected).val();
                });
                sendauthoritie(rr.toString(), $("#hfuser").val(), 1);

            });
            $("#LS1").click(function () {
                var rr = [];
                $('#urlist :selected').each(function (i, selected) {
                    var data = {
                        id: $(selected).val(),
                        text: $(selected).text()
                    };
                    ...
                    var newOption = new Option(data.text, data.id, false, false);
                    $('#rlist').append(newOption).trigger('change');
                    $(selected).remove();
                    rr[i] = $(selected).val();
                });
                
                sendauthoritie(rr.toString(), $("#hfuser").val(), 0);

            });
            $("#RSA").click(function () {
                var rr = [];
                $('#rlist > option').each(function (i, selected) {
                    var data = {
                        id: $(selected).val(),
                        text: $(selected).text()
                    };
            ...
            $("#LSA").click(function () {
                var rr = [];
                $('#urlist > option').each(function (i, selected) {
                    var data = {
                        id: $(selected).val(),
                        text: $(selected).text()
                    };
    ...
    <script>
        ...
        function sendauthoritie(a,u,c) {
            if (a != null && c != null && u!=null) {           
                ...
                xmlhttp.open("GET", "authoritie-user.php?a=" + a + "&u=" + u + "&c=" + c, true);
                xmlhttp.send();
