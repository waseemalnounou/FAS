...
{   
 $sql = "SELECT  rr.id,Role_Name from roleresources rr inner join Role ro ON rr.Role_Id=ro.id INNER join  resources re ON rr.Resoources_Id=re.Id      inner JOIN authorities a ON a.RR_Id=rr.Id where re.Resources_Name='contact' and a.User_ID=".$_SESSION['user_Id'];
 $result = mysqli_query($db, $sql);
 $count = mysqli_num_rows($result);  
    if ($count > 0) {
...
{
 $sql = "SELECT *,(select CONCAT(First_Name,' ',Last_Name) from user u where u.Id=a.User_ID) as UName FROM contact a";
 $result = mysqli_query($db, $sql);
 $count = mysqli_num_rows($result);  
    if ($count > 0) {
    // output data of each row
    while($row = mysqli_fetch_assoc($result)) {
        echo "<tr id='".$row["Id"]."' ><td>" . "<input type='radio'  class='btn btn-warning btn-lg btn-block' name='selectedrow' " . "value='".$row["Id"]."'/>". "</td><td data-value='Name' >". $row["UName"]. "</td><td data-value='type' >".  $row["Contact_Type"]."</td><td data-value='Value' >". $row["Value"]. "</td></tr>";
    }
  }
else
{
...
if(isset($_POST['UName'])&&isset($_POST['txta'])){
     $sql = "INSERT INTO contact(User_ID,Contact_Type,value)
    VALUES('".$_POST["UName"]."','".$_POST["txtt"]."','".$_POST["txta"]."')";
if (mysqli_query($db, $sql)) {
     $_SESSION["msg"] ="new contact is added";
     $_SESSION["typ"] ='s'; 
} else {
     $_SESSION["msg"] ="err:contact not added";
     $_SESSION["typ"] ='w';
}

    }     /* end add contact */ /*delete contact*/
...
if (mysqli_query($db, $sql)) {
     ...
     $_SESSION["typ"] ='w';
      ...
      } /* end delete contact */ /*edit contact*/
      elseif($_POST['submit']=='update'){
           $id= $_POST["hf"];
         if(isset($_POST['UName'])&&isset($_POST['txta'])&&isset($_POST['txtt'])){
     $sql = "UPDATE contact SET  User_ID='".$_POST["UName"]."',Contact_Type='".$_POST["txtt"]."',value='".$_POST["txta"]."' where Id='$id'";

if (mysqli_query($db, $sql)) {
    
      $_SESSION["msg"] ="contact information is updated";
      $_SESSION["typ"] ='s';
} else {
      $_SESSION["msg"] ="err:contact not update";
      $_SESSION["typ"] ='w';
            }
         } 
      }/*end edit contact*/
...
<!--add row -->
   <?php
                               if($dbins){ echo "<input type='button' id='btnmodel' class='btn btn-primary' value='new contact' data-toggle='modal' data-target='#myModal'/>";}    
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <!-- body -->                                                                                          
                                             ...
                                             <label >contact Type:</label><input class="form-control "  type="text" id="txtt" name="txtt"  placeholder="contact type" />           
                                             <label >contact:</label><input class="form-control "  type="text" id="txta" name="txta"  placeholder="contact" />
                                                  </div>
