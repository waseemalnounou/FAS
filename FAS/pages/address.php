...
{   
 $sql = "SELECT  rr.id,Role_Name from roleresources rr inner join Role ro ON rr.Role_Id=ro.id INNER join  resources re ON rr.Resoources_Id=re.Id      inner JOIN authorities a ON a.RR_Id=rr.Id where re.Resources_Name='address' and a.User_ID=".$_SESSION['user_Id'];
 $result = mysqli_query($db, $sql);
 $count = mysqli_num_rows($result);  
    if ($count > 0) {
...
else
{
 ...
 $count = mysqli_num_rows($result);  
    if ($count > 0) {
    // output data of each row
   echo "<select name='UName' class='form-control'><option>please select</option>";
    while($row = mysqli_fetch_assoc($result)) {
...
{
 $sql = "SELECT *,(select CONCAT(First_Name,' ',Last_Name) from user u where u.Id=a.User_ID) as UName FROM address a";
 $result = mysqli_query($db, $sql);
 $count = mysqli_num_rows($result);  
    if ($count > 0) {
    // output data of each row
    while($row = mysqli_fetch_assoc($result)) {
        echo "<tr id='".$row["Id"]."' ><td>" . "<input type='radio'  class='btn btn-warning btn-lg btn-block' name='selectedrow' " . "value='".$row["Id"]."'/>". "</td><td data-value='Name' >". $row["UName"]. "</td><td data-value='type' >".  $row["address_type"]."</td><td data-value='address' >". $row["address"]. "</td></tr>";
    }
  }
else
{
...
if(isset($_POST['UName'])&&isset($_POST['txta'])){
     $sql = "INSERT INTO address(User_ID,address_type,address)
    VALUES('".$_POST["UName"]."','".$_POST["txtt"]."','".$_POST["txta"]."')";
if (mysqli_query($db, $sql)) {
     $_SESSION["msg"] ="new address is added";
     $_SESSION["typ"] ='s'; 
} else {
     $_SESSION["msg"] ="err:address not added";
     $_SESSION["typ"] ='w';
}

    }     /* end add address */ /*delete address*/
...
if (mysqli_query($db, $sql)) {
     ...
     $_SESSION["typ"] ='w';
      ...
      } /* end delete address */ /*edit address*/
      elseif($_POST['submit']=='update'){
           $id= $_POST["hf"];
         if(isset($_POST['UName'])&&isset($_POST['txta'])&&isset($_POST['txtt'])){
     $sql = "UPDATE address SET  User_ID='".$_POST["UName"]."', address_type='".$_POST["txtt"]."' , address='".$_POST["txta"]."' where Id='$id'";

if (mysqli_query($db, $sql)) {
    
      $_SESSION["msg"] ="address information is updated";
      $_SESSION["typ"] ='s';
} else {
      $_SESSION["msg"] ="err:address not update";
      $_SESSION["typ"] ='w';
            }
         } 
      }/*end edit address*/
...
<!--add row -->
  <?php
                               if($dbins){ echo "<input type='button' id='btnmodel' class='btn btn-primary' value='new address' data-toggle='modal' data-target='#myModal'/>";}    
                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <!-- body -->                                                                                          
                                             ...
                                             <label >User Name:</label>  <?php  inituserlist($db) ?> 
                                             <label >address Type:</label><input class="form-control "  type="text" id="txtt" name="txtt"  placeholder="address type" />           
                                             <label >address:</label><input class="form-control "  type="text" id="txta" name="txta"  placeholder="address" />
                                                  </div>
...
?>
   ...
   $("#edt").click(function () {
                 ...
                 case 'UName':
                 $("#UName").val($(this).text()); break;
         case 'type':
                 $("#txtt").val($(this).text()); break;           
         case 'address':
                 $("#txta").val($(this).text()); break;            
