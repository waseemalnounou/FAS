...
{   
 $sql = "SELECT  rr.id,Role_Name from roleresources rr inner join Role ro ON rr.Role_Id=ro.id INNER join  resources re ON rr.Resoources_Id=re.Id      inner JOIN authorities a ON a.RR_Id=rr.Id where re.Resources_Name='role' and a.User_ID=".$_SESSION['user_Id'];
 $result = mysqli_query($db, $sql);
 $count = mysqli_num_rows($result);  
    if ($count > 0) {
...
{
 $sql = "SELECT  re.Resources_Name,re.Description,ro.Role_Name from roleresources rr inner join Role ro ON rr.Role_Id=ro.id INNER join  resources re ON rr.Resoources_Id=re.Id      inner JOIN authorities a ON a.RR_Id=rr.Id where a.User_ID=".$_SESSION['user_Id'];
 $result = mysqli_query($db, $sql);
 $count = mysqli_num_rows($result);  
    if ($count > 0) {
    // output data of each row
    while($row = mysqli_fetch_assoc($result)) {  
        echo "<tr><td data-value='Resources_Name'>". $row["Resources_Name"]."</td><td data-value='Description' >".  $row["Description"]. "</td><td data-value='Role_Name'>" . $row["Role_Name"]. "</td></tr>";
    }
  }
else
{
...
if($_POST["txtpw1"]==$_POST["txtpw2"]){
     $sql = "INSERT INTO role(First_Name,Last_Name,password,email,Birthday,Gender)
    VALUES('".$_POST["txtfn"]."','".$_POST["txtln"]."','".$_POST["txtpw1"]."','".$_POST["txtemail"]."','".$_POST["birthday"]."','".$_POST["gender"]."')";
if (mysqli_query($db, $sql)) {
     $_SESSION["msg"] ="new role is added";
     $_SESSION["typ"] ='s'; 
} else {
     $_SESSION["msg"] ="err:role not added";
     $_SESSION["typ"] ='w';
}
  }else{
...
if (mysqli_query($db, $sql)) {
     ...
     $_SESSION["typ"] ='w';
      ...
      } /* end delete role */ /*edit role*/
      elseif($_POST['submit']=='update'){
           $id= $_POST["hf"];
         if(isset($_POST['txtfn'])&&isset($_POST['txtln'])&&isset($_POST['birthday'])&isset($_POST['gender'])&&isset($_POST['txtemail'])){
     $sql = "UPDATE role SET  First_Name='".$_POST["txtfn"]."',Last_Name='".$_POST["txtln"]."',email='".$_POST["txtemail"]."',Birthday='".$_POST["birthday"]."',Gender='".$_POST["gender"]."' where Id='$id'";

if (mysqli_query($db, $sql)) {
    
      $_SESSION["msg"] ="role information is updated";
      $_SESSION["typ"] ='s';
} else {
      $_SESSION["msg"] ="err:role not update";
      $_SESSION["typ"] ='w';
            }
         } 
      }/*end edit role*/
...
<!--add row -->
                                <?php
                               if($dbins){ echo "<input type='button' id='btnmodel' class='btn btn-primary' value='new role' data-toggle='modal' data-target='#myModal'/>";}    
